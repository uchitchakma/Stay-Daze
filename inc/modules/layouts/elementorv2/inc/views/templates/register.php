                    ...
                    <div class="login-form-wrapper register-component active">
                        ...
                        <form action="#" class="form" method="post">
                            ...
                            <div class="st-icheck mt-2">
                             ...
                             <!-- Partner MOU (shown only when Partner is selected) -->
                             <!-- Partner MOU: same styles as privacy checkbox, hidden until Partner selected -->
                        <?php
                        $mou_page = get_page_by_path('partner-mou');
                        $mou_url = $mou_page ? get_permalink($mou_page) : site_url('/partner-mou');
                        ?>
                        <div class="st-icheck partner-mou" style="display:none; margin-top:10px;">
                            <div class="st-icheck-item">
                                <label for="partner_mou">
                                    <input id="partner_mou" type="checkbox" name="partner_mou_agree" class="mr5"> 
                                    <?php echo wp_kses( sprintf( __( 'I agree to the <a class="st-link" href="%s" target="_blank">Partner MOU</a>', 'traveler' ), esc_url( $mou_url ) ), ['a' => ['href' => [], 'class' => [], 'target' => []]] ); ?>
                                    <span class="checkmark fcheckbox"></span>
                                </label>
                            </div>
                        </div>
...
<!-- Toggle partner MOU visibility and require checkbox when Partner selected -->
<script>
document.addEventListener('DOMContentLoaded', function(){
    var mouWrapper = document.querySelector('.partner-mou');
    var mouCheckbox = document.getElementById('partner_mou');

    function toggleMou(){
        var partnerSelected = document.querySelector('input[name="register_as"]:checked') &&
            document.querySelector('input[name="register_as"]:checked').value === 'partner';
        if (mouWrapper) mouWrapper.style.display = partnerSelected ? 'block' : 'none';
        if (mouCheckbox) {
            if (partnerSelected) mouCheckbox.setAttribute('required','required');
            else mouCheckbox.removeAttribute('required');
        }
    }

    var radios = document.querySelectorAll('input[name="register_as"]');
    radios.forEach(function(r){ r.addEventListener('change', toggleMou); });
    toggleMou();
});
</script>
