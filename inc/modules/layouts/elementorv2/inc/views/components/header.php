		...
		if ( ! empty( $header_items ) and is_array( $header_items ) ) {
			...
			foreach ( $header_items as $key => $val ) {
				if ( ! empty( $val['header_item'] ) ) {
					...
					// New Code
					  if ( $val['header_item'] == 'login' ) {
							// Render account dropdown for logged-in users; for guests output the same markup/classes
							// used for styling but link to the login page (no popup trigger).
							if ( is_user_logged_in() ) {
								echo stt_elementorv2()->loadView( 'components/header-items/account' );
							} else {
								$login_page = get_permalink( st()->get_option( 'page_user_login' ) );
								echo '<li class="dropdown dropdown-user-dashboard d-none d-sm-block d-md-block st-header-account">';
								echo '<a href="' . esc_url( $login_page ) . '" class="dropdown-toggle" role="button" aria-expanded="false">';
								echo '<span class="stt-icon stt-icon-user1"></span>';
								echo '</a>';
								echo '</li>';
							}
						}
